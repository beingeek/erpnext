...
@frappe.whitelist()
def get_items(date1,item_group=None,qty_order=None,item_code=None):
	object_list=[]
	...
	if item_group==None or item_group=='':
		if item_code==None or item_code=='':
			item_list1=frappe.db.sql("""select name,item_name from `tabItem` where is_sales_item=1""")
		else:
			item_list1=frappe.db.sql("""select name,item_name from `tabItem` where is_sales_item=1 and item_code=%s""",item_code)
	else:
		if item_code==None or item_code=='':
			item_list1=frappe.db.sql("""select name,item_name from `tabItem` where item_group=%s and is_sales_item=1""",item_group)
		else:
			item_list1=frappe.db.sql("""select name,item_name from `tabItem` where item_group=%s and is_sales_item=1 and item_code=%s""",(item_group,item_code))
	...
	for row in item_list1:
		...
		infinity_purchase_qty=frappe.db.sql("""select sum(qty) from `tabPurchase Order Item` where item_code=%s and schedule_date>=%s and docstatus<2""",(row[0],date1))
		if not len(infinity_purchase_qty):
			inf_p_qty=0
		else:
			inf_p_qty=infinity_purchase_qty[0][0]
		...
		week_purchase_qty=frappe.db.sql("""select sum(qty) from `tabPurchase Order Item` where item_code=%s and schedule_date between %s and %s and docstatus<2""",(row[0],(date1),add_days(date1,5)))
		if not len(week_purchase_qty):
			week_p_qty=0
		else:
			week_p_qty=week_purchase_qty[0][0]
...
#po
	...
	if qty_order==None or qty_order=='' or qty_order=='Item Code':
		return sorted(object_list,key=itemgetter('item_code'),reverse = False)
	else:
...
def getPODetails(item_code,date_fil):
	...
	first1=frappe.db.sql("""select sum(pi.box) from `tabPurchase Order` as po inner join `tabPurchase Order Item` as pi on po.name=pi.parent where not po.status='Completed' and not po.status='Closed' and not po.status='To Bill' and not po.status='Cancelled' and  pi.item_code=%s and po.schedule_date=%s and pi.received_qty<=0""",(item_code,date_fil))
	second1=frappe.db.sql("""select sum(pi.box) from `tabPurchase Order` as po inner join `tabPurchase Order Item` as pi on po.name=pi.parent where not po.status='Completed' and not po.status='Closed' and not po.status='To Bill' and not po.status='Cancelled' and  pi.item_code=%s and po.schedule_date=%s and pi.received_qty<=0""",(item_code,add_days(date_fil,1)))
	third1=frappe.db.sql("""select sum(pi.box) from `tabPurchase Order` as po inner join `tabPurchase Order Item` as pi on po.name=pi.parent where not po.status='Completed' and not po.status='Closed' and not po.status='To Bill' and not po.status='Cancelled' and  pi.item_code=%s and po.schedule_date=%s and pi.received_qty<=0""",(item_code,add_days(date_fil,2)))
	four1=frappe.db.sql("""select sum(pi.box) from `tabPurchase Order` as po inner join `tabPurchase Order Item` as pi on po.name=pi.parent where not po.status='Completed' and not po.status='Closed' and not po.status='To Bill' and not po.status='Cancelled' and  pi.item_code=%s and po.schedule_date=%s and pi.received_qty<=0""",(item_code,add_days(date_fil,3)))
	five1=frappe.db.sql("""select sum(pi.box) from `tabPurchase Order` as po inner join `tabPurchase Order Item` as pi on po.name=pi.parent where not po.status='Completed' and not po.status='Closed' and not po.status='To Bill' and not po.status='Cancelled' and  pi.item_code=%s and po.schedule_date=%s and pi.received_qty<=0""",(item_code,add_days(date_fil,4)))
...
def addQtyAdjustLog(item_code,ordered_qty,allocated_qty,sales_order,back_qty):
	doc=frappe.get_doc({
		...
		"qty_surplus_and_shortage":str(flt(ordered_qty)-flt(allocated_qty))
